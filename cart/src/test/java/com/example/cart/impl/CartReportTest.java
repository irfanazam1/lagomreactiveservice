...
public class CartReportTest {
    ...
    public void createAReportOnFirstEvent() throws InterruptedException, ExecutionException, TimeoutException {
        ...
        feed(new CartEntity.ItemAdded(cartId, "abc", 2, eventTime));

        CartReport report = Await.result(reportRepository.findById(cartId));
    ...
    public void creationDateDoesNotChangeOnNewEvents() throws InterruptedException, ExecutionException, TimeoutException {
        String cartId = UUID.randomUUID().toString();
        Instant eventTime = Instant.now();
        feed(new CartEntity.ItemAdded(cartId, "abc", 1, eventTime));
        ...
        feed(new CartEntity.ItemAdded(cartId, "abc", 2, eventTime.plusSeconds(30)));

        CartReport updatedReport = Await.result(reportRepository.findById(cartId));
    ...
    public void checkoutDateIsSetOnCheckout() throws InterruptedException, ExecutionException, TimeoutException {
        ...
        feed(new CartEntity.CheckedOut(cartId, checkeoutTime));
    ...
    private void feed(CartEntity.Event event) throws InterruptedException, ExecutionException, TimeoutException {
